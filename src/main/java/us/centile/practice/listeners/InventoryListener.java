...
public class InventoryListener implements Listener
{
    ...
    public InventoryListener(final nPractice plugin) {
        this.plugin = plugin;
    }
        ...
        if (practicePlayer.getCurrentState() == PlayerState.LOBBY || practicePlayer.getCurrentState() == PlayerState.QUEUE) {
            ...
            if (invTitle.contains("unranked queue")) {
                ...
                if (kit != null) {
                    ...
                    if (party2 != null) {
                        ...
                        if (queueManager.isUnrankedPartyQueueEmpty(kitName)) {
                            ...
                            this.plugin.getManagerHandler().getPartyManager().notifyParty(party2, ChatColor.YELLOW + "Your party has been added to the " + ChatColor.GREEN + "Unranked 2v2 " + kitName + ChatColor.YELLOW + " queue!");
                        ...
                        player.sendMessage(ChatColor.YELLOW + "You have been added to the " + ChatColor.GOLD + "Unranked " + kitName + ChatColor.YELLOW + " queue.");
                        practicePlayer.setCurrentState(PlayerState.QUEUE);
                        player.getInventory().setContents(this.plugin.getManagerHandler().getItemManager().getQueueItems());
                        player.updateInventory();
                    }
                ...
                if (kit != null) {
                    ...
                    if (party2 != null) {
                        ...
                        if (queueManager.isRankedPartyQueueEmpty(kitName)) {
                            ...
                            this.plugin.getManagerHandler().getPartyManager().notifyParty(party2, ChatColor.YELLOW + "Your party has been added to the " + ChatColor.GREEN + "Ranked 2v2 " + kitName + ChatColor.YELLOW + " queue with " + ChatColor.GREEN + partyElo + " elo" + ChatColor.YELLOW + "!");
                            practicePlayer.setCurrentState(PlayerState.QUEUE);
                            ...
                            queueManager.addToPartyRankedQueue(kitName, player.getUniqueId());
                            this.plugin.getManagerHandler().getPartyManager().notifyParty(party2, ChatColor.YELLOW + "Party has been added to the " + ChatColor.GOLD + "Ranked " + kitName + ChatColor.YELLOW + " queue. " + ChatColor.GREEN + "[" + partyElo + "]");
                            practicePlayer.setCurrentState(PlayerState.QUEUE);
                            final PracticePlayer partyMember2 = this.plugin.getManagerHandler().getPracticePlayerManager().getPracticePlayer(party2.getMembers().get(0));
                            partyMember2.setCurrentState(PlayerState.QUEUE);
                            final Player partyPlayer2 = this.plugin.getServer().getPlayer(partyMember2.getUUID());
                        ...
                        if (queueManager.isRankedQueueEmpty(kitName)) {
                            ...
                            player.sendMessage(ChatColor.YELLOW + "You have been added to the " + ChatColor.GOLD + "Ranked " + kitName + ChatColor.YELLOW + " queue. " + ChatColor.GREEN + "[" + playerElo + "]");
                            practicePlayer.setCurrentState(PlayerState.QUEUE);
                            player.getInventory().setContents(this.plugin.getManagerHandler().getItemManager().getQueueItems());
                            player.updateInventory();
                        }
                            ...
                            for (final UUID queuedUuid3 : queueManager.getQueuedForRankedQueue(kitName)) {
                                ...
                                if (diff2 < 325) {
                                    ...
                                    final List<UUID> firstTeam3 = new ArrayList<UUID>();
                                    final List<UUID> secondTeam3 = new ArrayList<UUID>();
                    ...
                    if (queueManager.isPremiumQueueEmpty(kitName)) {
                        ...
                        for (final UUID queuedUuid4 : queueManager.getQueuedForPremiumQueue(kitName)) {
                            ...
                            if (diff3 < 325) {
                                ...
                                final List<UUID> firstTeam4 = new ArrayList<UUID>();
                                final List<UUID> secondTeam4 = new ArrayList<UUID>();
                ...
                if (kit != null) {
                    ...
                    final Party targetParty2 = this.plugin.getManagerHandler().getPartyManager().getParty(target.getUniqueId());
                    player.sendMessage(ChatColor.YELLOW + "Sent a duel request to " + ChatColor.GREEN + target.getName() + ((targetParty2 != null) ? (ChatColor.YELLOW + "'s party " + ChatColor.GREEN + "(" + (targetParty2.getMembers().size() + 1) + ")") : "") + ChatColor.YELLOW + " with the kit " + ChatColor.GREEN + kitName + ChatColor.YELLOW + "!");
                    this.plugin.getManagerHandler().getRequestManager().addDuelRequest(target, player, new DuelRequest(kitName));
                    final UtilActionMessage actionMessage = new UtilActionMessage();
                    actionMessage.addText(ChatColor.GREEN + player.getName() + ((party3 != null) ? (ChatColor.YELLOW + "'s party " + ChatColor.GREEN + "(" + (party3.getMembers().size() + 1) + ")") : "") + ChatColor.YELLOW + " has requested a duel with the kit " + ChatColor.GOLD + kitName + ChatColor.YELLOW + ".");
                ...
                if (kit != null) {
                    ...
                    player.closeInventory();
                    final List<UUID> firstTeam5 = new ArrayList<UUID>();
                    final List<UUID> secondTeam5 = new ArrayList<UUID>();
                ...
                this.plugin.getServer().broadcastMessage(ChatColor.RED.toString() + ChatColor.BOLD + (isPlayerTournament ? "PLAYER " : "") + "TOURNAMENT (" + tournament.getMaximumPerTeam() + "v" + tournament.getMaximumPerTeam() + ") IS STARTING SOON.");
                final UtilActionMessage actionMessage2 = new UtilActionMessage();
                actionMessage2.addText(ChatColor.YELLOW + "Join the tournament using the command /join or [Click Here]").addHoverText(ChatColor.GRAY + "Click to join the tournament").setClickEvent(UtilActionMessage.ClickableType.RunCommand, "/join " + Tournament.getTournaments().size());
                ...
                if (kit != null) {
                    player.closeInventory();
                    final Party party4 = this.plugin.getManagerHandler().getPartyManager().getParty(player.getUniqueId());
                    final List<UUID> members = new ArrayList<UUID>(party4.getMembers());
                ...
                final String kitName2 = this.plugin.getManagerHandler().getEditorManager().getPlayerEditingKit(player.getUniqueId());
                final Map<Integer, PlayerKit> kitMap = practicePlayer.getKitMap().get(kitName2);
                e.setCancelled(true);
                ...
                if (itemClicked.getType() == Material.CHEST && e.getSlot() < 9) {
                    ...
                    final ItemStack loadedKit = UtilItem.createItem(Material.ENDER_CHEST, 1, (short)0, ChatColor.YELLOW + "KIT: " + ChatColor.GREEN + kitName2 + " #" + kitIndex);
                    final ItemStack load = UtilItem.createItem(Material.BOOK, 1, (short)0, ChatColor.YELLOW + "Load/Edit Kit " + ChatColor.GREEN + kitName2 + " #" + kitIndex);
                    final ItemStack rename = UtilItem.createItem(Material.NAME_TAG, 1, (short)0, ChatColor.YELLOW + "Rename Kit " + ChatColor.GREEN + kitName2 + " #" + kitIndex);
                    final ItemStack delete = UtilItem.createItem(Material.FLINT, 1, (short)0, ChatColor.YELLOW + "Delete Kit " + ChatColor.GREEN + kitName2 + " #" + kitIndex);
                    final PlayerKit playerKit = new PlayerKit(kitName2, kitIndex, ChatColor.GREEN + "KIT: " + kitName2 + " #" + kitIndex, new ItemStack[0], new ItemStack[0]);
                    playerKit.setMainContents(this.plugin.getManagerHandler().getKitManager().getKit(kitName2).getMainContents());
                    playerKit.setArmorContents(this.plugin.getManagerHandler().getKitManager().getKit(kitName2).getArmorContents());
                    practicePlayer.addKit(kitName2, kitIndex, playerKit);
                    ...
                    if (kitMap != null && kitMap.containsKey(kitIndex)) {
                        ...
                        if (kitMap.get(kitIndex).getMainContents().length > 0) {
                            ...
                            player.getInventory().setContents(this.plugin.getManagerHandler().getKitManager().getKit(kitName2).getMainContents());
                            player.getInventory().setArmorContents(kitMap.get(kitIndex).getArmorContents());
                        }
                        player.updateInventory();
                        this.plugin.getManagerHandler().getKitManager().openEditiKitsInventory(player, this.plugin.getManagerHandler().getKitManager().getKit(kitName2), kitMap.get(kitIndex));
                    }
                }
                    ...
                    if (kitMap != null && kitMap.containsKey(kitIndex)) {
                        ...
                        final ItemStack save = UtilItem.createItem(Material.CHEST, 1, (short)0, ChatColor.YELLOW + "Create Kit " + ChatColor.GREEN + kitName2 + " #" + kitIndex);
