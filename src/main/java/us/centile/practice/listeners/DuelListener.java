...
public class DuelListener implements Listener
{
    ...
    public DuelListener(final nPractice plugin) {
    ...
    public void onDuelCreate(final DuelCreateEvent e) {
                ...
                if (duel.getDuelState() == DuelState.STARTING) {
                    if (--countdown == 0) {
                        ...
                        if (duel.getFfaPlayers() != null) {
                            for (final UUID uuid : duel.getFfaPlayersAlive()) {
                                ...
                                player.sendMessage(ChatColor.GREEN + "The match has started, good luck!");
                            }
                        }
                        else {
                    ...
                    if (duel.getFfaPlayers() != null) {
                        for (final UUID uuid : duel.getFfaPlayersAlive()) {
                            ...
                            player.sendMessage(ChatColor.YELLOW + "The match starts in " + ChatColor.GREEN + countdown + ChatColor.YELLOW + "...");
                        }
                    }
                    else {
                        for (final UUID uuid : duel.getFirstTeamAlive()) {
                    ...
                    countdown -= 2;
                    if (countdown <= 0) {
                        DuelListener.this.plugin.getServer().getPluginManager().callEvent(new DuelEndEvent(duel));
                    }
                }
        ...
        Bukkit.getScheduler().runTaskAsynchronously(this.plugin, new UpdateInventoryTask(this.plugin, UpdateInventoryTask.InventoryTaskType.SPECTATOR));
    ...
    public void onDuelEnd(final DuelEndingEvent e) {
        ...
        if (duel.getFfaPlayers() != null) {
            final Player lastPlayer = this.plugin.getServer().getPlayer(duel.getFfaPlayersAlive().get(0));
        ...
        for (final Player player5 : duelPlayers) {
            final String[] information = { ChatColor.DARK_GRAY.toString() + ChatColor.STRIKETHROUGH + "----------------------------------------------------", ChatColor.GOLD + "Match Results: " + ChatColor.GRAY + "(Clickable Inventories)" };
            player5.sendMessage(information);
            winnerMessage.sendToPlayer(player5);
            loserMessage.sendToPlayer(player5);
            player5.sendMessage(ChatColor.DARK_GRAY.toString() + ChatColor.STRIKETHROUGH + "----------------------------------------------------");
        }
    ...
    public void onDuelEnd(final DuelEndEvent e) {
        ...
        if (duel.getFfaPlayers() != null) {
            ...
            final Player player = this.plugin.getServer().getPlayer(duel.getFfaPlayersAlive().get(0));
            if (player == null) {
                return;
            }
            this.plugin.getManagerHandler().getPracticePlayerManager().sendToLobby(player);
