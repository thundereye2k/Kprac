...
public class KitManager extends Manager
{
    ...
    public void saveKits() {
        ...
        for (final Map.Entry<String, Kit> kitEntry : this.kitMap.entrySet()) {
            ...
            if (kit.getIcon() != null && kit.getMainContents() != null && kit.getArmorContents() != null) {
    ...
    public void openEditiKitsInventory(final Player player, final Kit defaultKit, final PlayerKit kit) {
        final Inventory inventory = Bukkit.createInventory(null, 54, "Editing Kit");
        ...
        for (final ItemStack armor : defaultKit.getArmorContents()) {
            ...
            meta.setLore(Arrays.asList("", ChatColor.RED + "This is automatically equipped"));
    ...
    private void saveMongo() {
        for (final Kit kit : this.kitMap.values()) {
            ...
            this.handler.getPlugin().getPracticeDatabase().getKits().replaceOne(Filters.eq("name", kit.getName()), document, new UpdateOptions().upsert(true));
        }
    }
}
