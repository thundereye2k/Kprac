...
public class ArenaManager extends Manager
{
    ...
    public Arena closest(final Location location) {
        ...
        for (final Arena arena : this.getArenaMap().values()) {
            ...
            if (location.getWorld() == arena.getSecondTeamLocation().getWorld() && arena.getSecondTeamLocation().distanceSquared(location) < distancee) {
    ...
    public Arena getRandomArena() {
        final List<Arena> arenas = this.arenaMap.values().stream().filter(Arena::isEnabled).filter(Arena::isOpen).collect(Collectors.toList());
        if (arenas.isEmpty()) {
            return null;
        }
