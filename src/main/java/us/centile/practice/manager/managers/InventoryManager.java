...
public class InventoryManager extends Manager
{
    ...
    public void setSpectatorInventory() {
        ...
        for (final Duel duel : this.handler.getDuelManager().getUuidIdentifierToDuel().values()) {
            ...
            if (duel.getFfaPlayers() != null && duel.getFfaPlayers().size() > 0) {
                firstTeam = Bukkit.getOfflinePlayer((UUID)duel.getFfaPlayers().get(0)).getName() + "'s Party";
            }
            else if (duel.getFirstTeam() != null && duel.getFirstTeam().size() >= 2) {
                firstTeam = Bukkit.getOfflinePlayer((UUID)duel.getFirstTeam().get(0)).getName() + "'s Party";
            }
            else {
                firstTeam = Bukkit.getOfflinePlayer((UUID)duel.getFirstTeam().get(0)).getName();
            }
            String secondTeam;
            if (duel.getFfaPlayers() != null && duel.getFfaPlayers().size() > 0) {
                secondTeam = Bukkit.getOfflinePlayer((UUID)duel.getFfaPlayers().get(0)).getName() + "'s Party";
            }
            else if (duel.getFirstTeam() != null && duel.getSecondTeam().size() >= 2) {
                secondTeam = Bukkit.getOfflinePlayer((UUID)duel.getSecondTeam().get(0)).getName() + "'s Party";
            }
            else {
                secondTeam = Bukkit.getOfflinePlayer((UUID)duel.getSecondTeam().get(0)).getName();
            }
    ...
    private void setPartyEventsInventory() {
        ...
        this.partyEventsInventory.addItem(new ItemStack[] { goldSword, slimeBall });
    }
